...
Stickshift.enabled = false

if Stickshift.enabled
  ActionController::Dispatcher.instrument :dispatch, :label => "Rails Dispatcher", :top_level => true
  ...
  ActionController::Routing::RouteSet.instrument :call
  ...
  ActiveRecord::ConnectionAdapters.constants.each do |c|
    ...
    if adapter_class < ActiveRecord::ConnectionAdapters::AbstractAdapter &&
        adapter_class.instance_methods.include?("execute")
